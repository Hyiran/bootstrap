...
public class UiAutomatorBridge {
  ...
  public UiAutomatorBridge() {
    try {
      ...
      uiAutomatorBridge = getField(UiDevice.class, FIELD_UI_AUTOMATOR_BRIDGE,
          device);
  ...
  public Display getDefaultDisplay() {
    return (Display) invoke(
        method(CLASS_UI_AUTOMATOR_BRIDGE, METHOD_GET_DEFAULT_DISPLAY),
        uiAutomatorBridge);
  }

  public InteractionController getInteractionController() {
    return new InteractionController(getField(CLASS_UI_AUTOMATOR_BRIDGE,
        FIELD_INTERACTION_CONTROLLER, uiAutomatorBridge));
  }

  public QueryController getQueryController() {
    return new QueryController(getField(CLASS_UI_AUTOMATOR_BRIDGE,
        FIELD_QUERY_CONTROLLER, uiAutomatorBridge));
  }

  public boolean injectInputEvent(final InputEvent event, final boolean sync) {
    return (Boolean) invoke(
        method(CLASS_UI_AUTOMATOR_BRIDGE, METHOD_INJECT_INPUT_EVENT,
            InputEvent.class, boolean.class), uiAutomatorBridge, event, sync);
  }
}
