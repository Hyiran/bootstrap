...
public class AndroidElement {
  ...
  public Point getAbsolutePosition(final Point point)
    ...
    return PositionHelper.getAbsolutePosition(point, rect, new Point(rect.left,
        rect.top), false);
  }
  ...
  public String getResourceId() throws UiObjectNotFoundException {
    ...
    try {
      /*
       * Unfortunately UiObject does not implement a getResourceId method. There
       * is currently no way to determine the resource-id of a given element
       * represented by UiObject. Until this support is added to UiAutomater, we
       * try to match the implementation pattern that is already used by
       * UiObject for getting attributes using reflection. The returned string
       * matches exactly what is displayed in the UiAutomater inspector.
       */
      final AccessibilityNodeInfo node = (AccessibilityNodeInfo) invoke(
          method(el.getClass(), "findAccessibilityNodeInfo", long.class), el,
  ...
  public boolean performMultiPointerGesture(final PointerCoords[]... touches) {
    try {
      if (API_18) {
        ...
        return (Boolean) invoke(
            method(el.getClass(), "performMultiPointerGesture",
                PointerCoords[][].class), el, (Object) touches);
      } else {
        Logger.error("Device does not support API < 18!");
        return false;
      }
  ...
  public boolean setText(final String text, final boolean unicodeKeyboard)
      throws UiObjectNotFoundException {
    if (unicodeKeyboard && UnicodeEncoder.needsEncoding(text)) {
      Logger.debug("Sending Unicode text to element: " + text);
      final String encodedText = UnicodeEncoder.encode(text);
